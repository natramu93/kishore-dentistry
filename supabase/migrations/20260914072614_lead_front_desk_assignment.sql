-- Keep lead creation transactional while routing intake to the branch front
-- desk resource. Operations and Front Office users create leads on behalf of
-- the branch, so the first active Front Office profile is the owning resource
-- for follow-up, booking and invoicing. A Front Office user remains the owner
-- of leads they enter themselves. If Operations creates a lead before a
-- branch has provisioned a Front Office profile, leave it in the open pool so
-- a future Front Office user can claim it.
...
begin
  ...
  if v_actor.role::text = 'front_office' then
    v_assignee_id := p_actor;
  elsif v_actor.role::text = 'operations' then
    select p.id
    into v_assignee_id
    from crm.profiles p
    join crm.user_branches ub on ub.user_id = p.id
    where ub.branch_id = p_branch_id
      and p.role::text = 'front_office'
      and p.is_active
...
) is 'Creates a branch-scoped lead, keeps Front Office intake with its actor, and routes Operations intake to the first active Front Office resource for that branch.';
